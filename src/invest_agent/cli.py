...
def committee_review_main(topic: str, symbols: str | None = None, *, hydrate_missing_data: bool = False) -> None:
    parsed_symbols = [item.strip() for item in (symbols or "").split(",") if item.strip()]
    result = CommitteeReviewService(get_store()).run_review(
        CommitteeReviewRunRequest(topic=topic, symbols=parsed_symbols, hydrate_missing_data=hydrate_missing_data),
        actor=RunCardActor.CLI,
    )
    print(json.dumps(_json(result), indent=2, ensure_ascii=False))
...
def main() -> None:
    ...
    if args.command == "committee-review":
        ...
        committee_review_main(args.topic, args.symbols, hydrate_missing_data=args.refresh)
